...
\midrule
9 & 6 & $6.83\times10^{-9}$ & $1.24\times10^{-3}$ & $2.41\times10^{-10}$ \\
\bottomrule
\end{tabular}
\end{table}

Second, a prescribed 100,000-state shock-tube workload samples $P_4/P_1$ and $T_4/T_1$ log-uniformly over the declared model domain. Table~\ref{tab:shocktube_workload} reports the resulting shock-strength distribution and total CPU time. The MLP is 14.3 times faster than one bracketed solve per state, with relative $L_2$ error 0.158\% and 100\% valid pressure bounds. This is a deterministic propagation benchmark rather than calibrated probabilistic uncertainty quantification.

\begin{table}[H]
\centering
\caption{Prescribed 100,000-state shock-tube workload. Distribution columns report $P_2/P_1$.}
...
\midrule
Bracketed Brent & 1.706 & 4.2596 & 1.3773 & 3.7274 & 9.0311 \\
Physics-guided MLP & 0.119 & 4.2594 & 1.3772 & 3.7260 & 9.0334 \\
\bottomrule
\end{tabular}
\end{table}
