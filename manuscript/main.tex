...
The contributions of this work are as follows. First, a unified surrogate workflow is formulated for five canonical compressible thermal-fluid problems. Second, forward, inverse, and hybrid AI--analytical learning tasks are separated explicitly so that each network has a physically meaningful role. Third, branch decomposition, \rev{positive and bounded transformations}, and boundary \rev{envelopes} are used to preserve non-bijective and singular behavior. Finally, the models are validated against analytical solutions using direct comparison, relative-error analysis, and thermodynamic consistency checks. \rev{The revision further provides controlled naive-versus-structured ablations, classical interpolation and bracketed-root baselines, measured timing, edge-holdout tests, training-seed variability, and a two-to-five-dimensional extension of the same shock-tube compatibility relation. These additions evaluate the numerical role of the surrogate without making interpolation the central theme of the article.}
...
\end{equation}
which are then used by an optimizer such as Adam to update the parameters iteratively until convergence~\cite{Adam}. In this way, the network progressively adjusts its internal representation so as to reduce the prediction error over the training dataset.
...
To clarify the role of each surrogate model, Table~\ref{tab:io_summary} summarizes the principal input and output variables for all learning tasks considered in this study. Because several of the benchmark problems require both forward and inverse mappings, the table distinguishes these tasks explicitly and also indicates the main preprocessing or reformulation adopted for each case.
...
\captionof{table}{Summary of the principal input and output parameters used in the surrogate models for each gas-dynamics problem.}
...
& Forward
& $M$
& $\{T/T^*,\,P/P^*,\,\rho/\rho^*,\,u/u^*,\,P_0/P_0^*,\,4fL^*/D\}$
...
\begin{table}[t]
\centering
\caption{Reproducible training and evaluation contract used for the revised results. Inputs and transformed targets are standardized using training-set statistics.}
...
The Rayleigh-flow learning pipeline is summarized in Figure~\ref{fig:rayleigh_flowchart}. Exact analytical relations are first sampled to generate the synthetic dataset used for supervised learning. For the forward problem, the surrogate receives an engineered feature vector built from the Mach number and predicts the thermodynamic ratios of interest. For the inverse problem, the non-bijective mapping is resolved through branch decomposition, and two separate expert networks are trained for the subsonic and supersonic regimes. This structure avoids the ambiguity associated with the multi-valued inverse temperature relation and enables direct recovery of the physically admissible Mach number. The AI-predicted pressure and temperature ratios are further used to reconstruct the Rayleigh line on the $T$--$s$ diagram.
...
\node[process, below=of gen] (feat) {Feature engineering for forward model\\ $[M,\;M^2,\;1/M,\;1/M^2]$ + scaling};
...
To quantify the forward behavior, Figure \ref{fig:rayleigh_error} displays the pointwise relative error for each predicted variable across the Mach-number range. \rev{The separate inverse blind-grid norms are reported in Table~\ref{tab:primary_metrics}; the revised text does not infer global robustness from a visual overlap alone.}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Rayleigh_Error_Revised.pdf}
    \caption{Relative error analysis for Rayleigh flow predictions. The error distribution is centered around zero with minimal variance, confirming the robustness of the ML model across the entire Mach regime.}
    \label{fig:rayleigh_error}
\end{figure}

\subsubsection{The Inverse Problem}
The inverse problem—determining the Mach number for a given temperature ratio—is a common task in gas-dynamics analysis. Since analytical inversion of the implicit Rayleigh equations is unavailable in a convenient single-valued form, users typically rely on tables or iterative solvers. Our "Expert Networks" approach provides a direct branch-declared solution. Figure \ref{fig:rayleigh_inverse} visualizes this inverse mapping. The grey line represents the analytical solution, while the red and blue dashed lines show the predictions of the subsonic and supersonic networks, respectively. \rev{The revised inverse uses the stagnation-temperature relation:}
...
\end{align}
\rev{The revised experiment uses 900 analytical training states over $0.2\le M\le5.0$, combining logarithmic Mach sampling with additional points near $M=1$. The inverse test grid contains 1,040 independent states and is logarithmically refined near the sonic limit.}
...
\begin{revision}
The revised forward formulation imposes the known sonic structure explicitly,
\begin{equation}
...
Figure~\ref{fig:fanno_flowchart} summarizes the workflow adopted for Fanno flow. Because the friction parameter spans several orders of magnitude and vanishes quadratically at the sonic state, the learning pipeline combines feature engineering, the exact positive envelope in Equation~\ref{eq:fanno_positive_transform}, and regime decomposition. For the inverse problem, the regularized square-root coordinate is supplied to two independently trained bounded experts.
...
\node[process, below=of gen] (feat) {Feature engineering and scaling\\ Forward features: e.g. $[M,1/M,\log M]$ or\\ $[M,M^2,1/M,\log M]$};
...
Figure \ref{fig:fanno_ratios} compares the revised direct neural-network predictions with the analytical Fanno ratios over $0.2\le M\le5.0$. Quantitative blind inverse errors and physical diagnostics are reported in Tables~\ref{tab:primary_metrics} and~\ref{tab:ablation_physics} rather than inferred from visual overlap alone.
...
To address this ill-posedness and eliminate iterative costs, we propose a domain-decomposed neural approach. The solution space is split into subsonic and supersonic regimes. \rev{Two specialized experts learn $\mathcal{F}^{-1}:\sqrt{4fL^*/D}\rightarrow M$ and map their latent outputs into their respective admissible Mach intervals. The square-root input follows the local quadratic sonic behavior and remains finite at choking.}
...
\begin{revision}
In the submitted prototype, analytical endpoint samples acted as \emph{soft} data anchors; they were not hard constraints. The revised direct model instead defines $q=(\beta-\mu)/(\pi/2-\mu)$ and
\begin{equation}
...
The workflow used for the oblique-shock surrogate is summarized in Figure~\ref{fig:oblique_flowchart}. Because the $\beta$--$\theta$--$M$ manifold is bounded by strict physical limits and exhibits solution duality, the learning pipeline combines analytical data generation, an exact direct-map envelope, and separate weak/strong inverse outputs.
...
\begin{figure}[H]
    ...
    \caption{The revised $\beta$--$\theta$--$M$ diagram. Dashed curves are the hard-envelope surrogate and solid curves are the analytical relation; both physical endpoints are imposed exactly.}
...
To bypass the iterative computational cost and provide immediate design feedback, we formulated the shock location problem as a direct regression task. We trained a Deep Neural Network (DNN) to approximate the inverse function $\mathcal{F}^{-1}$:

\begin{equation}
    \frac{A_s}{A_t} \approx \text{DNN}\left( \frac{A_e}{A_t}, \frac{P_b}{P_{01}} \right)
\end{equation}
...
The workflow adopted for the convergent-divergent nozzle surrogate is summarized in Figure~\ref{fig:nozzle_flowchart}. Instead of learning the full flow field directly, the neural network is trained as an inverse shock-location predictor that maps nozzle geometry and back pressure to the shock position inside the divergent section.
...
As shown in Figure~\ref{fig:nozzle_flowchart}, the nozzle problem is formulated as an inverse regression task rather than a full field-prediction problem. Synthetic training data are generated by sampling a nozzle area ratio and an admissible internal shock location, then evaluating the corresponding back-pressure ratio through the exact sequence of isentropic expansion, normal-shock compression, and subsonic diffusion. The neural network is subsequently trained to approximate the inverse mapping from geometry and back pressure to shock location. Once trained, the model enables both immediate shock-position prediction and rapid post-processing tasks such as design-space heatmap generation and analytical pressure-distribution reconstruction.
...
\begin{itemize}
    ...
    \item he Throat Limit ($A_s \to A_t$): As back pressure increases (moving up the y-axis), the shock is pushed upstream towards the throat. The heatmap clearly shows the boundary where the shock vanishes and the nozzle unchokes (First Critical Point).
...
\begin{figure}[H]
    ...
    \caption{Design-space map retained from the submitted manuscript. Nozzle exit-area ratio is on the horizontal axis, back-pressure ratio on the vertical axis, and color denotes the predicted internal shock location.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{pressure_comparison_final (1).png}
...
In this architecture, a lightweight Deep Neural Network serves as a direct function approximator for the inverse of the shock tube equation. The network takes the initial conditions ($P_4/P_1, T_4/T_1$) as inputs and predicts the shock strength $P_2/P_1$. \rev{Both inputs vary in the revised dataset: $P_4/P_1\in[1.5,300]$ and $T_4/T_1\in[0.5,2]$.}
...
The workflow used for the shock-tube surrogate is summarized in Figure~\ref{fig:shocktube_flowchart}. Rather than predicting the full flow field directly, the neural network learns only the implicit algebraic step associated with shock strength, while the complete spatiotemporal solution is recovered analytically.
...
\node[process, below=of data] (prep) {Input transformation and scaling\\ e.g. $\log_{10}(P_4/P_1)$};
...
The resulting surrogates are most useful in many-query settings. Table~\ref{tab:baseline_timing} reports warm-started median CPU times from seven repeats for 5,000 queries. Batched MLP inference is 96--1057 times faster than repeated Brent solves, while interpolation remains approximately 4--11 times faster than the MLP. Offline analytical data generation and model fitting are excluded from these online timings and must be amortized.

\begin{table}[H]
\centering
\caption{Measured median CPU time for 5,000 queries.}
...
The Python scripts for data generation, neural-network training, inference, post-processing, and the dimensional scaling audit are available from the project repository: \url{https://github.com/Ehsan-Roohi/GasDynamicsSciML}. The repository contains the generated CSV evidence and tests for all five cases.
