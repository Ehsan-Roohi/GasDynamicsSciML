...
Thank you for the careful and constructive review of ``Physics-Guided Neural Surrogates for Canonical Compressible Thermal-Fluid Relations.'' We retained the structure and exposition of the submitted manuscript and made targeted corrections and additions within that exact source. New and revised text is shown in blue; a clean version is also supplied. All new numerical claims come from a public, one-command repository with unit tests and machine-readable CSV evidence. Incorrect or inconsistent learned mappings were regenerated as vector figures, while benchmark summaries are presented as tables rather than dashboard-style plots. The Rayleigh pointwise-error figure was replaced by a compact norm table. Only the overall workflow diagram remains in the article; five problem-specific diagrams are preserved as editable TikZ and vector PDF in the repository. We have removed qualitative overclaims and explicitly report negative results, classical baselines, coverage, training variability, and limitations.
...
\item Added an analytical-Jacobian nozzle inverse-design demonstration and a prescribed 100,000-state shock-tube workload; both are reported as compact tables and CSV evidence.
\item Named the released five-problem suite GasDynBench and added a compact benchmark card without changing the article's main theme.
\item Removed five problem-specific workflow diagrams from the article to reduce length and preserved their editable sources in \texttt{docs/workflows/}.
...
\response{We removed the former ``indicative'' timing table. Table 9 now reports warm-up, seven-repeat median CPU timings, with IQR stored in \texttt{timing.csv}. For 5,000 queries the MLP is 96--1057 times faster than repeated bracketed roots, but interpolation is 4--11 times faster than the MLP. We now state explicitly that, for a single query, Brent is comparable and is faster for Rayleigh and oblique inversion; the neural advantage is a batching effect. Table 10 reports dimensional storage, and Table 12 gives a prescribed 100,000-state shock-tube workload: 1.706~s for Brent versus 0.119~s for the MLP, with 0.158\% relative $L_2$ error. Offline training cost is excluded and remains explicit.}
...
The public GasDynBench project is \url{https://github.com/Ehsan-Roohi/GasDynamicsSciML}. It contains the package, analytical tests, CI workflow, configuration, exact CSV evidence, vector figures, highlighted and clean manuscript sources, and this response letter. The moved workflow diagrams are in \texttt{docs/workflows/}; the application audits are reproduced by \texttt{scripts/run\_application\_audits.py}. No quantitative value is inserted into the manuscript without corresponding generated evidence.
